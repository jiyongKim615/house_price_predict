...
from uncertainty_estimation.lgb_utils import *
...
# When a custom objective is provided LightGBM doesn't know its response function so the user is responsible for making
...
# transformed predictions since the objective is defined by LightGBM. However, when a custom objective is also provided
...
class Expectile2():
    ...
    def n_dist_param():
        ...
        """
        n_param = len(Expectile2.expectiles)
        return n_param
    ...
    def param_dict():
        ...
        """
        param_dict = []
        for i in range(len(Expectile2.expectiles)):
            param_dict.append({"expectile_" + str(Expectile2.expectiles[i]): identity})
    ...
    def expectile_pnorm(tau, m=0, sd=1):
        """Normal Expectile Distribution Function. For more details and lgb_distributions see https://rdrr.io/cran/expectreg/man/enorm.html
    ...
    def expect_norm(tau, m=0, sd=1):
        """Calculate expectiles from Normal distribution for given tau values. For more details and lgb_distributions see https://rdrr.io/cran/expectreg/man/enorm.html
        ...
        while (diff > 1e-10) and (index < 1000):
            root = Expectile2.expectile_pnorm(zz) - tau
    ...
    def gradient_expectile(y: np.ndarray, expectile: np.ndarray, tau: np.ndarray, weights: np.ndarray):
        ...
        grad = stabilize_derivative(grad, Expectile2.stabilize)
        grad = grad * (-1) * weights
        return grad
    ...
    def hessian_expectile(y: np.ndarray, expectile: np.ndarray, tau: np.ndarray, weights: np.ndarray):
        ...
        hes = stabilize_derivative(hes, Expectile2.stabilize)
        hes = hes * (-1) * weights
        return hes
    ...
    def Dist_Objective(predt: np.ndarray, data: lgb.Dataset):
        ...
        preds_expectile = predt.reshape(-1, Expectile2.n_dist_param(), order="F")
        ...
        grad = np.zeros(shape=(len(target), len(Expectile2.expectiles)))
        hess = np.zeros(shape=(len(target), len(Expectile2.expectiles)))

        for i in range(len(Expectile2.expectiles)):
            grad[:, i] = Expectile2.gradient_expectile(y=target,
                                                      expectile=preds_expectile[:, i],
                                                      tau=Expectile2.expectiles[i],
                                                      weights=weights
                                                      )

            hess[:, i] = Expectile2.hessian_expectile(y=target,
                                                     expectile=preds_expectile[:, i],
                                                     tau=Expectile2.expectiles[i],
                                                     weights=weights
                                                     )
    ...
    def Dist_Metric(predt: np.ndarray, data: lgb.Dataset):
        ...
        for i in range(len(Expectile2.expectiles)):
            loss_expectile.append(
                Expectile2.expectile_loss(y=target,
                                         expectile=preds_expectile[:, i],
                                         tau=Expectile2.expectiles[i]
                                         )
            )
